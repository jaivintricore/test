	...
	IKeysManager.prototype._getContent = function(){
		...
		if(this._showAddKeyForm === true){
			...
			content += '<tr>';
			content += '<td>Host: </td><td><input type="text"  name="key_form_host" id="key_form_host" size="60" />'
	...
	IKeysManager.prototype._updateButtons = function(){
		...
		if(this._showAddKeyForm === false){
			...
			this._getDialog().addButton('Activate', function(){
				...
				var error = IKeysManager.validateHost($('key_form_host').value);
				if(error !== true){
					errors.push(error);
				}
				if(errors.length > 0){
					...
					this._attch._host = IKeysManager.getHost($('key_form_host').value);
